...
from ..llm import Provider, count_tokens
...
class Memory:
    ...
    def budget(self, session: Session, system_prompt: str) -> int:
        ...
        return max(0, session.context_window - sys_tokens - self.cfg.loop.response_reserve_tokens)
    ...
    def _as_message(turn: Turn) -> dict:
        if turn.role == "assistant":
            return (
                ...
                else {"role": "assistant", "content": str(turn.content)}
            )
        if turn.role == "tool":
            c = turn.content
    ...
    def maybe_summarize(self, session: Session, system_prompt: str) -> bool:
        ...
        keep_n = max(1, ceil(self.cfg.memory.summary_keep_ratio * len(active)))
