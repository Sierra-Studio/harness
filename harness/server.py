...
def _event_to_dict(ev) -> dict:
    ...
    if ev.kind == "tool_start":
        return {"kind": "tool_start", "name": ev.name,
                "args": ev.args, "call_id": ev.call_id}
    if ev.kind == "tool_result":
        return {"kind": "tool_result", "name": ev.name,
                "content": ev.content, "call_id": ev.call_id}
    return {"kind": ev.kind}
...
def build_harness() -> Harness:
    ...
    for srv in mcp_http_servers():
        try:
            h.add_mcp_http(srv["url"], srv["name"], srv["headers"],
                           oauth=srv.get("oauth"))
...
def serve(host: str = "127.0.0.1", port: int = 8800) -> int:
    ...
    provider = "OpenRouter" if h.cfg.openrouter_api_key else "FakeProvider (offline)"
    print(f"Harness SSE server on http://{host}:{port} · repo={backend} · "
          f"provider={provider} · model={h.cfg.model}")
