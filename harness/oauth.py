...
class OAuthConfig:
    ...
    cache_dir: Path = field(
        default_factory=lambda: Path.home() / ".harness" / "mcp-auth")
...
class _CallbackHandler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):  # noqa: N802
        ...
        self.end_headers()
        self.wfile.write(b"<html><body><h3>Authorization complete.</h3>"
...
class OAuthClient:
    ...
    def __init__(self, http_client, cfg: Optional[OAuthConfig] = None):
    ...
    def _discover(self, resource_url: str, www_authenticate: str) -> dict:
        ...
        prm = None
        for url in filter(None, [parse_resource_metadata_url(www_authenticate),
        ...
        meta = None
        for url in [as_url.rstrip("/") + "/.well-known/oauth-authorization-server",
        ...
        if not meta:
            raise RuntimeError(
                f"could not discover OAuth metadata for {resource_url}")
        meta["_resource"] = resource
        return meta

    def _register(self, meta: dict, cache: dict) -> tuple[str, Optional[str]]:
        ...
        if not reg_ep:
            raise RuntimeError(
                ...
                "set a client_id out of band")
        body = {"client_name": self.cfg.client_name,
    ...
    def _authorize(self, resource_url: str, www_authenticate: str, cache: dict) -> dict:
        ...
        token_req = {"grant_type": "authorization_code", "code": code,
                     ...
                     "code_verifier": verifier, "resource": meta["_resource"]}
        if client_secret:
